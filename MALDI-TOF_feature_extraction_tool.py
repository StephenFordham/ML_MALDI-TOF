...
def _epilog():
    return ("\nHow to run? It's easy, just follow the terminal examples listed.\n"
            ...
            "\npython MALDI_TOF_feature_extraction_tool.py -f directory_with_raw_maldi_csv_files \n"
            "\nHow to run specific mass-to-charge (m/z) trimming? Simply state your cut-off m/z values"
            "\npython MALDI-TOF_feature_extraction_tool.py --folder directory_with_raw_maldi_csv_files --mz_start 2000 --mz_end 20000\n"
            "\nTo run feature extraction and antibiotic label matching, run the following:"
            "\npython MALDI_TOF_feature_extraction_tool.py --folder directory_with_raw_maldi_csv_files --labels labels_dir\n"
            )


parser = argparse.ArgumentParser(description='\nDESCRIPTION:\n'
                                             '\nA MALDI-TOF MS 3-dalton feature extraction binning pre-processing tool '
                                             '\nfrom bacterial spectra samples to support Machine Learning (ML) applications'
                                             '\nFeatures are extracted from the 2-20 kilo-daltons m/z range\n'
...
parser.add_argument(
    ...
    type=str,
    default='MALDI-TOF-MS_Features')

parser.add_argument(
    "-b",
    "--bin_size",
    help="Feature extraction bin size",
    type=int,
    default=3
)
...
def extraction():
    ...
    s = list(range(args.mz_start, args.mz_end, args.bin_size))
    e = list(range((args.mz_start + args.bin_size), (args.mz_end + args.bin_size), args.bin_size))
    ...
    for spectrum in spectra_files:
        ...
        if spectrum.endswith('.csv'):
            ...
            start_spectra_val = args.mz_start
            stop_value = args.mz_end
            end_spectra_val = args.mz_start + args.bin_size
            intensity_m_z_vals = []
            ...
            for i in range(len(df)):
                ...
                if start_spectra_val < stop_value:
                    ...
                    intensity_sum_dalton_window = df_spectra['intensity'].sum()
                    intensity_m_z_vals.append(intensity_sum_dalton_window)
                    start_spectra_val += args.bin_size
                    end_spectra_val += args.bin_size
